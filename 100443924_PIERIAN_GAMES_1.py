...
cur = conn.cursor()
cur.execute("SET search_path TO '100443924_PIERIAN_GAMES_1',public;")
...
sno NOT IN ( SELECT sno FROM ticket WHERE tno NOT IN ( SELECT tno FROM cancel));"
    ...
    # Delete an event. All the tickets for the event must be cancelled before an event can be deleted.
        ...
        query_d =(f"DELETE FROM event WHERE ecode ='{(value[0])}';")
        cur.execute(query_d)
        # Ensure that query functioning properly
        verify = f"select * from event where ecode = '{(value[0])}';"
        cur.execute(verify)
    ...
    # TO run this query_e following pre-requisition data in the respective table
        ...
        writeOutput("TASK " + key + "\n")
        query_e = (f"INSERT INTO ticket (tno, ecode, sno) SELECT COALESCE(MAX(tno), 0) + 1,"
                    f"'{value[0]}',{int(value[1])} FROM ticket WHERE NOT EXISTS ( SELECT 1 FROM ticket where  "
                    f" ecode = '{value[0]}' and sno = {int(value[1])});")
        cur.execute(query_e)
        # Ensure that query functioning properly
        verify = f"select * from ticket;"
        cur.execute(verify)
        writeOutput(cur.statusmessage + "\n")
    ...
    # on each date an event is held at a location.
        ...
        query_f = (f"SELECT event.elocation AS LOCATION, event.edate AS EVENT_DATE, "
                    f"COUNT(spectator.sno) AS TOTAL_SPECTATORS FROM event "
                    f"LEFT JOIN ticket  ON event.ecode = ticket.ecode "
                    f"LEFT JOIN spectator  ON ticket.sno = spectator.sno "
    ...
    # Present the data in event description sequence.
        ...
        writeOutput("TASK " + key + "\n")
        query_g = (f"SELECT event.edesc AS EVENT_DESCRIPTION,COUNT(ticket.tno) AS TOTAL_TICKETS_ISSUED "
                    f"FROM event LEFT JOIN ticket  ON event.ecode = ticket.ecode "
                    f"GROUP BY event.edesc ORDER BY event.edesc;")
        cur.execute(query_g)
        # Ensure that query functioning properly
        writeOutput(cur.statusmessage + "\n")
    ...
    # description of each event for which the spectator has been issued a ticket.
        ...
        query_i = (f"SELECT spectator.sname AS SPECTATOR_NAME, event.edate AS EVENT_DATE,"
                    f"event.elocation AS EVENT_LOCATION,event.etime AS EVENT_TIME,"
                    f"event.edesc AS EVENT_DISCRIPTION FROM spectator "
                    f"JOIN ticket  ON spectator.sno = ticket.sno "
    ...
    # the event code for the ticket and indicate if the ticket is valid or is cancelled.
        ...
        query_j = (f"SELECT spectator.sname AS SPECTATOR_NAME, ticket.ecode AS EVENT_CODE,"
                    ...
                    f"JOIN spectator  ON ticket.sno = spectator.sno "
                    f"LEFT JOIN cancel  ON ticket.tno = cancel.tno WHERE ticket.tno = '{int(value[0])}';")
        cur.execute(query_j)
        # Ensure that query functioning properly
        writeOutput(cur.statusmessage + "\n")
    ...
    # View the details of all cancelled tickets for a specific event.
        ...
        writeOutput("TASK " + key + "\n")
        query_k = (f"SELECT * FROM cancel where ecode ='{value[0]}';")
        cur.execute(query_k)
        writeOutput(cur.statusmessage + "\n")
    ...
    # Delete the contents of the database tables.
        ...
        writeOutput("TASK " + key + "\n")
        query_L = (f"DELETE FROM cancel; "
                   f"DELETE FROM ticket; "
                   f"DELETE FROM spectator; "
                   f"DELETE FROM event;")
        cur.execute(query_L)
        # Ensure that query functioning properly
